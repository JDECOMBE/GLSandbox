...
public class BatchRenderingMesh : IRenderingItem
{
    ...
    public BatchRenderingMesh(string filename, int count, Buffer<float> instanceVbo)
    {
        _filename = filename;
        InstanceCount = count;
    ...
    public BatchRenderingMesh(string fileName, int count, Vector3[] positions, Vector3[] colors, float[] scales, float[] randomValue)
    {
        _filename = fileName;
        InstanceCount = count;
